        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          {statusGroups.map((status) => {
            ...
            return (
              <div
                key={status}
                className={`bg-white rounded-xl p-6 border-2 ${statusConfig[status as keyof typeof statusConfig].borderColor} hover:shadow-lg transition-shadow cursor-pointer`}
                onClick={() => setSelectedStatus(status)}
              >
                <div className="flex items-center justify-between mb-4">
                  <div className={`w-12 h-12 rounded-lg ${statusConfig[status as keyof typeof statusConfig].bgColor} flex items-center justify-center`}>
                    <StatusIcon className={`w-6 h-6 ${statusConfig[status as keyof typeof statusConfig].color}`} />
                  </div>
                  <span className={`px-3 py-1 rounded-full ${statusConfig[status as keyof typeof statusConfig].badgeColor}`}>
                    {count}
                  </span>
                </div>
                    ...
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        <div className="flex items-center gap-3 mb-3">
                          <div className={`w-10 h-10 rounded-lg ${statusConfig[task.status as keyof typeof statusConfig].bgColor} flex items-center justify-center`}>
                            <StatusIcon className={`w-5 h-5 ${statusConfig[task.status as keyof typeof statusConfig].color}`} />
                          </div>
                          <div>
                            <h3 className="text-gray-900 mb-1">{task.title}</h3>
                            <span className={`inline-flex items-center gap-1 px-3 py-1 rounded-full ${statusConfig[task.status as keyof typeof statusConfig].badgeColor}`}>
                              {task.status}
                            </span>
                          </div>
                        </div>
            ...
            return (
              <div key={status} className="bg-white rounded-xl border border-gray-200 overflow-hidden">
                ...
                <button
                  ...
                  className={`w-full p-4 ${statusConfig[status as keyof typeof statusConfig].bgColor} border-b ${statusConfig[status as keyof typeof statusConfig].borderColor} flex items-center justify-between hover:opacity-80 transition-opacity`}
                >
